...
from apps.events.constants import TicketTypeMessages
...
def test_create_ticket_type_with_valid_data(organizer_client, event_factory):
    ...
    url = reverse_lazy(TICKET_TYPE_LIST, kwargs={"event_pk": event.id})
...
def test_list_ticket_types_for_event(api_client, event_factory, ticket_type_factory):
    ...
    url = reverse_lazy(TICKET_TYPE_LIST, kwargs={"event_pk": event.pk})
